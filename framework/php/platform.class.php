...
class PlatfomHelp {
	...
	public static function CheckBrowser() {
		...
		switch(true){
			...
			break;
			case (preg_match('/MQQBrowser/i',$user_agent));
			$mobile_browser = 'MQQBrowser';
			break;
			case (preg_match ( '/Mozilla/i', $user_agent )) && ! preg_match ( '/iuc/i', $user_agent );
			$mobile_browser = 'Mozilla';
			break;
			case (preg_match('/iuc/i',$user_agent));
			$mobile_browser = 'iuc';
			break;
			default;
			$mobile_browser = 'Normal';
			break;
		}
		return $mobile_browser;
	}

	public static function mobile_device_detect($iphone=true,$ipad=true,$android=true,$symbian=true,$opera=true,$blackberry=true,$palm=true,$windows=true,$mobileredirect=false,$desktopredirect=false){
		...
		switch(true){
			...
			case (preg_match('/(iris|3g_t|windows ce|opera mobi|windows ce; smartphone;|windows ce; iemobile |WPDesktop)/i',$user_agent));
			...
			break;
			case (preg_match('/windows nt/i',$user_agent));
			$mobile_browser = false;
			...
			case (preg_match('/ios/i',$user_agent));
			...
			$mobile_browser = $symbian;
			$status = 'Symbian';
			if(substr($symbian,0,4)=='http'){
				$mobileredirect = $symbian;
			}
			break;

			case (preg_match('/opera mini/i',$user_agent));
			$operamini_phone_ua = $_SERVER['HTTP_X_OPERAMINI_PHONE_UA'];
			if (preg_match('/symbian/i',$operamini_phone_ua)) {
				$mobile_browser = $symbian;
				$status = 'Symbian';
				if(substr($symbian,0,4)=='http'){
					$mobileredirect = $symbian;
			...
			case (preg_match('/windows phone os/i',$user_agent));
			$mobile_browser = $windows;
			$status = 'Windows phone';
			if(substr($windows,0,4)=='http'){
				$mobileredirect = $windows;
			}
			break;
