...
class DataBaseModel extends DataBase {
	...
	public function getTableName($table = '') {
		...
		if($table != '') {
			...
			return $table;
		} // 无表名 且 未定义表名 且 调用类属于数据类
		else if($this->_tableName == '' && is_subclass_of($this, 'DataBaseModel')) {
			// 获取当前类的类名
			$class = get_class($this);
			// 获取类名中 Model前面部分 当表名
			$table = strtolower(str_replace('Model', '', $class));
			// 返回 前缀+表名
			return $this->_tablePrefix . $table;
		}
...
//		$this->_autoData = array();
	...
	public function getRowCache($arr = array(), $_is_cache = false, $_cache_time = CACHE_LIFETIME_SQL) {
		...
		$sql = $this->_compile_select(false);
	 ...
	 * @return mixed
	 */
	public function execCache($sql, $_is_cache = falsee, $_cache_time = CACHE_LIFETIME_SQL) {
		return $this->_getCache($sql, 'fetchAll', $_is_cache, $_cache_time);
	}
	...
	public function _getCache($sql, $fun = 'fetchRow', $_is_cache = false, $_cache_time = CACHE_LIFETIME_SQL) {
		$startime = BaseHelp::getMillisecond();
...
//								 ->setex($cache_sql, $_cache_time, json_encode($rtn)) : '';
			$exetime = (BaseHelp::getMillisecond() - $startime) / 1000;
