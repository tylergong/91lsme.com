...
//					</xml>";
	...
	private function search($keyword, $wxid) {
		...
		if(stripos($keyword, '@') === false) {
			...
			if($keyword == 'help' || $keyword == '帮助') {
				...
				$res['tracks'][0]['picUrl'] = 'http://www.91lsme.com/app/web/templates/public/image/dt2.png';
	...
	private function getWeatherInfo($cityName) {
		...
		foreach($data->results[0]->weather_data as $k => $v) {
			if($k == 0) {
				...
				$rtn[$k]['description'] = '';
				$rtn[$k]['picUrl'] = 'http://www.91lsme.com/app/web/templates/public/image/tq.png';
				$rtn[$k]['url'] = '';
			} else {
	...
	private function getNearPlace($wxid, $keyword) {
		...
		if($res['error'] == 0) {
			...
			$once['description'] = '';
			$once['picUrl'] = 'http://www.91lsme.com/app/web/templates/public/image/fj.png';
			$once['url'] = '';
